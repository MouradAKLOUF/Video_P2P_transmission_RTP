function [reception, isDecodable,numLostPkts,numPktsAvailable]= check_decodable_images_3(reception,TS_r,dt,Dictionary)

list_rec_pckts=getAllPackets(reception);
reception = clear_buffer(reception) ;                      %get_buffer(reception, 'bit_size')

if TS_r~=0
    temp_list=[];temp_others=[];
...
num_pkts=Dictionary(int16(TS_r/dt)+1,2);

if length(list_rec_pckts)==0
    isDecodable=false;
    numLostPkts=num_pkts;
    numPktsAvailable=0;
    return;
end
...
if last_rtp==true
    numLostPkts=num_pkts-length(curr_img);
